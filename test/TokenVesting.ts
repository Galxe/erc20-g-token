...
describe("Vesting", function () {
  ...
  async function deployVesting(
    ...
    const vesting = await VestingFactory.deploy(beneficiary, await token.getAddress(), start, duration, count);
  ...
  function vestTimePoints(start: number, duration: number, count: number): number[] {
    let rv = [];
  ...
  describe("deploy", function () {
    it("non-zero token address", async function () {
      ...
      const VestingFactory = await ethers.getContractFactory("TokenVesting");
      await expect(
        VestingFactory.deploy((await generateRandomWallet()).address, ethers.ZeroAddress, latestTime, 1000, 1000),
    ...
    it("invalid vesting parameters", async function () {
      ...
      const VestingFactory = await ethers.getContractFactory("TokenVesting");
      await expect(VestingFactory.deploy(deployer.address, tokenAddress, latestTime, 0, 1)).to.be.rejectedWith(
        "InvalidDuration",
      );
      await expect(VestingFactory.deploy(deployer.address, tokenAddress, latestTime, 100, 0)).to.be.rejectedWith(
        "InvalidNumVestings",
      );
      // overflow
      await expect(
    ...
    it("correct parameters", async function () {
      ...
      const tokenAddress = await token.getAddress();
      const VestingFactory = await ethers.getContractFactory("TokenVesting");
      await expect(VestingFactory.deploy(deployer.address, tokenAddress, latestTime, 100, 1)).not.to.be.reverted;
    });
  });
  ...
  describe("Correct vesting", function () {
    it("1-time vesting, 1000 wei token, 100 seconds", async function () {
      ...
      const vesting = await deployVesting(deployer, beneficiary.address, token, latestTime, duration, nVest, 1000);
      ...
      await expect(vesting.connect(randomPpl).release()).to.be.revertedWithCustomError(vesting, "NoTokenReleasable");
      ...
      // withdraw
      await expect(vesting.connect(randomPpl).release()).not.to.be.reverted;
      // post-withdraw
  ...
  describe("recover other ERC20", function () {
    it("non-owner cannot recover", async function () {
      const { deployer, token } = await loadFixture(deployVestingFixture);
      const latestTime = await time.latest();
      const startAt = latestTime + 200;
      const duration = 1000;
      ...
      await expect(vesting.connect(randomPpl).recoverOtherERC20(token, 1000)).to.be.revertedWithCustomError(
        vesting,
    ...
    it("invalid token", async function () {
      ...
      await expect(vesting.connect(beneficiary).recoverOtherERC20(token, 1000)).to.be.revertedWithCustomError(
        vesting,
        "InvalidRecoverTokenAddress",
      );
    });

    it("successful recover", async function () {
      const { deployer, token } = await loadFixture(deployVestingFixture);
      const latestTime = await time.latest();
      const startAt = latestTime + 200;
      const duration = 1000;
      ...
      await vesting.connect(beneficiary).recoverOtherERC20(otherToken, 1000);
      expect(await otherToken.balanceOf(beneficiary)).to.equal(1000);
    });
  });

  describe("recover native token", function () {
    it("non-owner cannot recover", async function () {
      const { deployer, token } = await loadFixture(deployVestingFixture);
      const latestTime = await time.latest();
      const startAt = latestTime + 200;
      ...
      await expect(vesting.connect(randomPpl).recoverNativeToken()).to.be.revertedWithCustomError(
        vesting,
        "OwnableUnauthorizedAccount",
      );
    });
    ...
    it("successful recover", async function () {
      ...
      const beneficiary = await generateRandomWallet();
      const vesting = await deployVesting(deployer, beneficiary.address, token, startAt, duration, nVest, 1000);
      await vesting.connect(beneficiary).recoverNativeToken();
    });
  });
});
